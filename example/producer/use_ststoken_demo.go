...
func main() {
	...
	// If you need to close the channel, please use close(producerConfig.StsTokenShutDown)
	producerConfig.StsTokenShutDown = make(chan struct{})
	producerConfig.StsTokenExpireTime = "1h" // Set the expiration time to 1 hour after the current time.
